...
UART_HandleTypeDef *shell_uart = NULL;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle)
{
    ...
    chry_ringbuffer_write_byte(&shell_rb, uart_rx_data);
    xEventGroupSetBitsFromISR(event_hdl, 0x10, &xHigherPriorityTaskWoken);
    portYIELD_FROM_ISR(xHigherPriorityTaskWoken);

    HAL_UART_Receive_IT(shell_uart, (uint8_t *)&uart_rx_data, 1);
}
...
static uint16_t csh_sput_cb(chry_readline_t *rl, const void *data, uint16_t size)
{
    ...
    HAL_UART_Transmit(shell_uart, (uint8_t *)data, size, 1000);
    return size;
}
